...
async def run(page, context):
    ...
    result = await page.evaluate("Array.from(document.querySelectorAll('img')).map(img => ({alt: img.alt || null, src: img.src, width: img.naturalWidth, height: img.naturalHeight}))")
