...
The primary interface to the browser. Takes a **natural language description** of what to do — the AI agent interprets and executes it.
...
The AI agent uses built-in tools (navigate, click, fill, page_perceive, find_element, etc.) to figure out and execute the right sequence of Playwright operations. After a successful exploration, it generates a replay script so subsequent runs can skip the AI and execute at full speed.
...
After `ai.action()` succeeds in explore mode, a replay script is auto-generated at `<case_dir>/scripts/<step_name>.py`. The script can be:
