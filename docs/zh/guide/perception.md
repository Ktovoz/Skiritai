...
Skiritai 的感知层通过 [browser-use](https://github.com/browser-use/browser-use) 的 CDP DOM 序列化引擎，为 AI Agent 提供结构化的页面理解。

## 概述

不依赖 Playwright 的 `page.evaluate()` 进行 DOM 遍历，感知层使用 Chrome DevTools Protocol (CDP) 将整个页面序列化为结构化的可交互元素树——每个元素都带有 CSS 选择器、文本、角色和属性等注释。
