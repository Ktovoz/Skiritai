...
class TestEndToEndExecution:
    ...
    def test_single_step_explore_success(self):
        ...
        with patch("app.engine.ai_context.run_agent", AsyncMock(return_value=mock_result)):
    ...
    def test_step_failure_stops_execution(self):
        ...
        with patch("app.engine.ai_context.run_agent", side_effect=mock_agent):
            report = asyncio.run(case.run())
...
class TestEventSequencing:
    ...
    def test_step_failed_event_on_failure(self):
        ...
        try:
            with patch("app.engine.ai_context.run_agent", AsyncMock(return_value=fail_result)):
                asyncio.run(case.run())
        finally:
            event_bus.unsubscribe(capture)
    ...
    def test_tool_called_events_published(self):
        ...
        try:
            with patch("app.engine.ai_context.run_agent", AsyncMock(return_value=agent_result)):
                asyncio.run(case.run())
        finally:
            event_bus.unsubscribe(capture)
...
class TestStepModeBehavior:
    ...
    def test_explore_mode_always_calls_agent(self):
        ...
        with patch("app.engine.ai_context.run_agent", AsyncMock(return_value=mock_result)) as mock_agent:
            report = asyncio.run(case.run())
...
class TestAPILifecycle:
    ...
    def test_run_mocked_then_query_results(self):
        ...
        with patch("app.routers.cases.run_case", new_callable=AsyncMock) as mock_run:
    ...
    def test_run_cancels_previous_execution(self):
        ...
        with patch("app.routers.cases.run_case", side_effect=slow_run):
