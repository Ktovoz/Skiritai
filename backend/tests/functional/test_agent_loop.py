...
class TestBuildAgent:
    ...
    def test_build_agent_returns_agent(self):
        from app.engine.llm.registry import _PROVIDERS
        ...
        with patch("app.engine.agent_loop.get_provider", return_value=mock_provider):
            with patch(
                "app.engine.agent_loop.create_react_agent"
            ) as mock_create:
    ...
    def test_build_agent_includes_perception_tools(self):
        ...
        import app.engine.tools        # registers Playwright action tools
        import app.engine.perception   # registers DOM perception tools
...
class TestRunAgent:
    ...
    def test_run_agent_success(self):
        ...
        with patch(
            "app.engine.agent_loop.build_agent"
        ) as mock_build:
            mock_agent = MagicMock()
            mock_agent.astream = self._mock_agent_astream
            mock_build.return_value = mock_agent

            with patch("app.engine.agent_loop.set_page") as mock_set_page:
                result = asyncio.run(
    ...
    def test_run_agent_with_url_parameter(self):
        ...
        with patch("app.engine.agent_loop.build_agent") as mock_build:
            mock_agent = MagicMock()
